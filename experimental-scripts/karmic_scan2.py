# -*- coding: utf-8 -*-
"""
Created on Sun Jun 22 16:43:04 2025
...
def explosion_event(t, y, params):
    return np.max(y) - 50  # Umbral más bajo para detectar divergencia antes
...
# =================================================================
# CLASIFICACIÓN AVANZADA (sin cambios)
# =================================================================
def classify_attractor(t, y, params):
    ...
    # =================================================================
    # MEJORA EN CRITERIOS DE CLASIFICACIÓN
    # =================================================================
    # Umbrales más flexibles para detectar caos
    chaos_threshold = 0.03 if n_peaks > 5 else 0.05  # Más sensible para oscilaciones complejas
    ...
    if n_peaks >= 3 and 'period_cv' in metadata and metadata['period_cv'] < 0.2:  # Mayor tolerancia
        return "Ciclo límite", metadata
    
    # Caos determinista (criterio menos estricto)
    if lyap_exp > chaos_threshold and lyap_r2 > 0.6 and metadata['spectral_entropy'] > 2.0 and autocorr_decay < 0.7:
        return "Caos determinista", metadata
    
    # Comportamiento cuasiperiódico
    if n_peaks >= 3 and metadata['spectral_entropy'] > 1.5 and metadata['spectral_entropy'] < 3.5 and autocorr_decay > 0.6:
        return "Cuasiperiódico", metadata
    
    # Atractor extraño (no caótico pero complejo)
    if metadata['spectral_entropy'] > 2.5 and autocorr_decay < 0.4:
...
# =================================================================
# SIMULACIÓN MONTE CARLO - PARÁMETROS OPTIMIZADOS
# =================================================================
def monte_carlo_simulation(n_simulations=100, t_span=(0, 500)):
    results = []
    
    # Rangos ajustados para reducir divergencia y favorecer otros atractores
    param_ranges = {
        'alpha_I': (0.001, 0.03),   # Reducción significativa de crecimiento
        'alpha_A': (0.001, 0.02),
        'alpha_V': (0.001, 0.02),
        'beta_IA': (0.2, 0.8),       # Interacciones más suaves
        'beta_AV': (0.2, 0.8),
        'beta_VI': (0.2, 0.8),
        'gamma_I': (0.5, 0.95),      # Mayor disipación
        'gamma_A': (0.5, 0.95),
        'gamma_V': (0.5, 0.95),
        'w': (0.35, 0.65)            # Rango de sabiduría ampliado
    }
    ...
    for i in tqdm(range(n_simulations), desc="Simulando"):
        ...
        # Condiciones iniciales más bajas y balanceadas
        y0 = np.random.dirichlet([1, 1, 1]) * np.random.uniform(0.01, 0.1)  # Valores iniciales más pequeños
        
        try:
            # Simulación de prueba más robusta
            sol_test = solve_ivp(
                three_roots_model,
                [0, 0.5],  # Ventana temporal más amplia
                y0,
                args=(params,),
                max_step=0.01,
                rtol=1e-6,
            ...
            if np.any(sol_test.y > 20):  # Umbral más bajo
                results.append({
                    'params': params,
                    'y0': y0,
            ...
            results.append({
                'params': params,
                'y0': y0,
                'attractor_type': 'Error en prueba',
        ...
        try:
            # Simulación principal con parámetros de tolerancia ajustados
            sol = solve_ivp(
                three_roots_model,
                t_span,
                y0,
                ...
                rtol=1e-7,  # Tolerancia más estricta
                atol=1e-9
            )
...
# =================================================================
# ANÁLISIS Y VISUALIZACIÓN (sin cambios)
# =================================================================
...
if __name__ == "__main__":
    ...
    test_results = monte_carlo_simulation(n_simulations=500)  # Más simulaciones para mejor estadística
