# -*- coding: utf-8 -*-
"""
Created on Sun Jun 22 20:17:27 2025
...
# -*- coding: utf-8 -*-
"""
Modelo Dinámico de Tres Raíces con Análisis Mejorado de Atractores
@author: eggra
"""
...
# =================================================================
# CLASIFICACIÓN AVANZADA DE ATRACTORES (MEJORADA)
# =================================================================
def classify_attractor(t, y, params):
    ...
    if np.max(y) > 20 or np.any(np.isnan(y)):
    ...
    fixed_point_threshold = max(0.005 * mean_value, 0.002 * amplitude)
    ...
    if len(autocorr) > 20:
        ...
        autocorr_periodic = np.max(autocorr[10:20]) > 0.7
    ...
    # 6. Detección de picos (mejorada para ciclos límite)
    n_peaks = 0
    peak_height = np.mean(V_ss) + 0.1 * amplitude
    try:
        peaks, properties = find_peaks(
            V_ss, 
            height=peak_height, 
            prominence=0.05 * amplitude,
            distance=max(3, int(0.02 * len(V_ss))))
        n_peaks = len(peaks)
    ...
    chaos_threshold = max(0.01, 0.03 * amplitude)
    ...
    if n_peaks >= 2 and 'period_cv' in metadata and metadata['period_cv'] < 0.15 and autocorr_periodic:
    ...
    # 5. Punto fijo estable (segundo chequeo)
    if std_dev < 2 * fixed_point_threshold:
        return "Punto fijo estable", metadata
...
# =================================================================
# SIMULACIÓN MONTE CARLO (OPTIMIZADA PARA PUNTOS FIJOS Y CICLOS)
# =================================================================
def monte_carlo_simulation(n_simulations=100, t_span=(0, 500)):
    results = []
    
    # Rangos optimizados para favorecer puntos fijos y ciclos límite
    param_ranges = {
        'alpha_I': (0.0001, 0.005),  # Tasas de crecimiento muy bajas
        'alpha_A': (0.0001, 0.005),
        'alpha_V': (0.0001, 0.005),
        'beta_IA': (0.01, 0.2),      # Interacciones débiles
        'beta_AV': (0.01, 0.2),
        'beta_VI': (0.01, 0.2),
        'gamma_I': (0.3, 0.8),       # Disipación media-alta
        'gamma_A': (0.3, 0.8),
        'gamma_V': (0.3, 0.8),
        'w': (0.4, 0.9)              # Sabiduría media-alta
    }
    ...
    for i in tqdm(range(n_simulations), desc="Simulando"):
        ...
        # Condiciones iniciales balanceadas
        y0 = np.random.dirichlet([1, 1, 1]) * np.random.uniform(0.05, 0.2)
        
        try:
            # Simulación con parámetros de tolerancia ajustados
            sol = solve_ivp(
                three_roots_model,
                t_span,
                y0,
...
if __name__ == "__main__":
    ...
    test_results = monte_carlo_simulation(n_simulations=1000)  # Más simulaciones para mejor estadística
