# -*- coding: utf-8 -*-
"""
Created on Sun Jun 15 20:33:39 2025
...
# -*- coding: utf-8 -*-
"""
Simulación Kármica con Detección de Comportamientos Diversos
"""
...
# ===== PARÁMETROS DE RESONANCIA KÁRMICA =====
t_max = 500  # Tiempo extendido para capturar patrones complejos
dt = 0.02    # Paso temporal ultra-fino
n_sim = 8000  # Simulaciones reducidas para mayor profundidad
checkpoint_interval = 50
partial_save_interval = 5

# Condiciones iniciales en equilibrio crítico
X0 = np.array([[0.75, 0.25, 0.00, 0.00],  # avidyā 
               [0.65, 0.35, 0.00, 0.00],  # rāga
               [0.55, 0.45, 0.00, 0.00]]) # dveṣa

# Parámetros resonantes (frecuencias naturales desintonizadas)
sigma = np.array([0.8, 1.2, 1.0])    # Tasas de maduración armónicas
gamma = np.array([0.15, 0.10, 0.20]) # Tasas de purga complementarias
omega = np.array([0.05, 0.03, 0.07]) # Recaída modulada
kappa = 0.85                         # Protección media
lambda_g = 0.80                      # Aprendizaje medio

# Matriz de acoplamiento resonante (retroalimentación fase-desfase)
alpha = np.array([
    [2.0, 1.8, 0.2],  # avidyā sincronizado con rāga
    [1.8, 2.0, 1.2],  # rāga como nodo central
    [0.2, 1.2, 2.0]   # dveṣa acoplado a rāga
])

# Ruidos sintonizados por componente
noise_params = {
    'D_beta': np.array([0.4, 0.6, 0.3]),  # Ruido diferenciado
    'b_zeta': np.array([0.4, 0.5, 0.3]),  # Ruido en maduración
    'a_eta': np.array([1.8, 1.5, 2.0]),   # Perfiles de ruido distintos
    'lambda_xi': np.array([0.4, 0.6, 0.3]) 
}
...
# ===== FUNCIONES DE ANÁLISIS DE CAOS CORREGIDAS =====
...
def approximate_entropy(U, m=2, r=0.2, sample_size=200):
    """Entropía aproximada optimizada para memoria - VERSIÓN CORREGIDA"""
    if len(U) < 100:
        return 0.0
...
def detect_behavior_type(trajectory, metrics):
    ...
    # 2. Comportamiento caótico (ya detectado por Lyapunov)
    ...
    # Promedio de picos por unidad de tiempo
    peaks_per_time = np.mean(n_peaks) / (t_max / 10)  # Normalizado por cada 10 unidades de tiempo
    
    # 3.1 Ciclo simple: pocos picos y periodicidad clara
    if peaks_per_time < 0.5 and np.all(n_peaks < 10):
        # Verificar periodicidad con FFT
        is_periodic = True
        for i in range(3):
            yf = fft(trajectory[:, i])
            ...
            # Contar picos significativos en el espectro
            significant_peaks = find_peaks(power_spectrum, height=np.max(power_spectrum)*0.1)[0]
            if len(significant_peaks) > 3:  # Demasiados picos para ser periódico simple
                is_periodic = False
                break
    ...
    if peaks_per_time > 1.0 and np.any(n_peaks > 15):
        return 'cyclic_complex'
    
    # 5. Comportamiento no clasificado
    return 'other'
...
try:
    for sim in tqdm(range(start_sim, n_sim), initial=start_sim, total=n_sim, 
        ...
        for i in range(3):
            ...
            # Exponente de Lyapunov (versión mejorada)
        ...
        # Guardar trayectorias representativas de cada tipo
        if behavior_type != 'other':
            # Incrementar contador para este tipo de comportamiento
            behavior_counters[behavior_type] += 1
            counter = behavior_counters[behavior_type]
            
            # Guardar solo las primeras 5 de cada tipo para no saturar
            if counter <= 5:
        ...
        # Monitoreo especial de Lyapunov
        if sim % 10 == 0:
            current_lyapunov = results[:sim+1, :, 4]
            mean_lyap = np.mean(current_lyapunov)
            max_lyap = np.max(current_lyapunov)
            print(f"\nSim {sim}: Lyapunov avg = {mean_lyap:.4f} | Max = {max_lyap:.4f}")
    ...
    import traceback
    traceback.print_exc()  # Imprimir el traceback completo para diagnóstico
