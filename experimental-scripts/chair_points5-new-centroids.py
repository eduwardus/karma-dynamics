...
# =============================================================================
# DATOS DE REINOS EXISTENCIALES (CORRECCIÓN PROPORCIONAL)
# =============================================================================
reinos = ['Devas', 'Asuras', 'Humanos', 'Animales', 'Pretas', 'Infiernos']
datos_reinos = np.array([
    [8.4, 3.8, 3.5],    # Devas: [Ignorancia, Aversión, Apego]
    [8.0, 15.2, 9.8],   # Asuras
...
# Reordenar a: [Ignorancia (I), Apego (A), Aversión (V)]
datos_reordenados = datos_reinos[:, [0, 2, 1]]

# Escalado proporcional global (factor único para mantener relaciones)
max_global = np.max(datos_reinos)
factor_escala = 3.0 / max_global  # Escalar al rango [0, 3]
realm_centroids_scaled = datos_reordenados * factor_escala

print("Centroides de reinos escalados proporcionalmente:")
for i, nombre in enumerate(reinos):
    print(f"{nombre}: I={realm_centroids_scaled[i, 0]:.2f}, A={realm_centroids_scaled[i, 1]:.2f}, V={realm_centroids_scaled[i, 2]:.2f}")
...
# =============================================================================
# INTERPRETACIÓN ACTUALIZADA CON ESCALADO PROPORCIONAL
# =============================================================================
def interpret_cluster(centroid, eigenvalues, eigenvectors):
    ...
    # 3. Asignación a reino por proximidad a centroides
    distances = np.linalg.norm(realm_centroids_scaled - centroid, axis=1)
    min_index = np.argmin(distances)
    realm = reinos[min_index]
    interpretation.append(f"Reino: {realm} (Dist: {distances[min_index]:.2f})")
    
    # 4. Características del estado
    if I > 2.0:
        interpretation.append("Confusión profunda")
    elif I < 0.8:
        interpretation.append("Sabiduría incipiente")
    
    if A > 2.0:
        interpretation.append("Apego compulsivo")
    elif A < 0.8:
        interpretation.append("Desapego emergente")
    
    if V > 2.0:
        interpretation.append("Aversión intensa")
    elif V < 0.8:
        interpretation.append("Ecuanimidad")
    
    # 5. Patrones especiales basados en proporciones
    if I > 1.5 and A > 1.5 and V > 1.5:
        interpretation.append("Triple veneno activo")
    elif I < 1.0 and A < 1.0 and V < 1.0:
        interpretation.append("Estado de claridad")
    elif A > V * 1.5 and A > I * 1.5:
        interpretation.append("Dominancia de apego")
    elif V > A * 1.5 and V > I * 1.5:
        interpretation.append("Dominancia de aversión")
    elif I > A * 1.5 and I > V * 1.5:
        interpretation.append("Dominancia de ignorancia")
...
def cluster_analysis(saddle_points, eigenvalues, eigenvectors):
    ...
    # Marcar centroides de reinos con estrellas grandes
    for i, (x, y, z) in enumerate(realm_centroids_scaled):
        ax2.scatter(x, y, z, s=200, marker='*', 
                   label=reinos[i], edgecolor='black')
    ...
    for label in unique_labels:
        ...
        # Calcular distancia a cada reino
        realm_distances = np.linalg.norm(realm_centroids_scaled - centroid, axis=1)
        closest_realm = reinos[np.argmin(realm_distances)]
        min_distance = np.min(realm_distances)
        ...
        cluster_stats.append({
            ...
            'closest_realm': closest_realm,
            'realm_distance': min_distance,
...
if __name__ == "__main__":
    ...
    print(f"Centroides de referencia (rango [0, 3]):")
    for i, nombre in enumerate(reinos):
        print(f"- {nombre}: I={realm_centroids_scaled[i, 0]:.2f}, A={realm_centroids_scaled[i, 1]:.2f}, V={realm_centroids_scaled[i, 2]:.2f}")
    ...
    if saddle_points:
        ...
        if not cluster_df.empty:
            ...
            for _, row in cluster_df.iterrows():
                ...
                print(f"Reino más cercano: {row['closest_realm']} (Distancia: {row['realm_distance']:.3f})")
