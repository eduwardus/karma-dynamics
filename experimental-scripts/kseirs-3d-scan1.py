# -*- coding: utf-8 -*-
"""
Created on Sun Jun 15 15:15:34 2025
...
sigma = np.array([0.5, 0.5, 0.5])     # Tasa base de maduración
gamma = np.array([0.2, 0.2, 0.2])     # Tasa base de purga
omega = np.array([0.1, 0.1, 0.1])     # Tasa base de recaída
kappa = 0.3                            # Factor de protección experiencial
lambda_g = 0.4                         # Factor de aprendizaje
...
noise_params = {
    'D_beta': np.array([0.05, 0.05, 0.05]),
    'b_zeta': np.array([0.1, 0.1, 0.1]),
...
def lyapunov_estimate(trajectory, min_points=100):
    """Estimación rápida de exponente de Lyapunov (versión corregida)"""
...
try:
    for sim in tqdm(range(start_sim, n_sim), initial=start_sim, total=n_sim, 
        ...
        # Guardar punto de control periódicamente
