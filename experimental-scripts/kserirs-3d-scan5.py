# -*- coding: utf-8 -*-
"""
Created on Sun Jun 15 21:16:58 2025
...
# -*- coding: utf-8 -*-
"""
Simulación Kármica con Parámetros para Ciclos Sostenidos (Ecuaciones Originales)
"""
...
# ===== PARÁMETROS AJUSTADOS PARA CICLOS SOSTENIDOS =====
t_max = 500  # Tiempo extendido
dt = 0.02    # Paso temporal
n_sim = 5000  # Número de simulaciones
checkpoint_interval = 50
partial_save_interval = 10

# Condiciones iniciales (pueden ajustarse para diversidad)
X0 = np.array([[0.75, 0.25, 0.00, 0.00],  # avidyā 
               [0.65, 0.35, 0.00, 0.00],  # rāga
               [0.55, 0.45, 0.00, 0.00]]) # dveṣa

# Parámetros clave ajustados:
sigma = np.array([0.8, 1.2, 1.0])      # Tasas de maduración
gamma = np.array([0.25, 0.20, 0.30])   # Tasas de purga aumentadas
omega = np.array([0.20, 0.18, 0.22])   # Tasas de recaída aumentadas
kappa = 0.7                            # Factor de protección reducido
lambda_g = -0.5                        # Factor de aprendizaje reducido

# Matriz de acoplamiento: valores más moderados
alpha = np.array([
    [1.2, 0.8, 0.5],
    [0.8, 1.5, 0.6],
    [0.5, 0.6, 1.3]
...
# Ruidos: mantener para variabilidad
noise_params = {
    'D_beta': np.array([0.1, 0.1, 0.1]),
    'b_zeta': np.array([0.4, 0.4, 0.4]),
    'a_eta': np.array([1.5, 1.5, 1.5]),
    'lambda_xi': np.array([0.3, 0.4, 0.3])
}
...
def correlation_dimension(traj, emb_dim=3, sample_size=1000):
    ...
    return coeffs[0]
    # ... (implementación idéntica a versiones anteriores) ...
...
def detect_behavior_type(trajectory, metrics):
    ...
    # 5. Comportamiento estable (no oscilante)
    if np.all(np.diff(trajectory[-100:,:], axis=0) < 0.001) and np.all(trajectory[-1,:] > 0.05):
        return 'stable'
...
try:
    for sim in tqdm(range(start_sim, n_sim), initial=start_sim, total=n_sim, 
        ...
        if behavior_type in behavior_counters:
            ...
            # Guardar hasta 10 trayectorias de cada tipo (excepto 'other')
            if behavior_type != 'other' and counter <= 10:
                filename = f"{behavior_type}_trajectory_sim_{sim}.npy"
                np.save(filename, full_trajectory)
                print(f"\nTrayectoria {behavior_type} detectada (sim={sim}, contador={counter})")
        
        # Monitoreo de progreso
        if sim % 20 == 0:
            print(f"\nSim {sim}: Comportamientos -> ", end="")
            for bt, count in behavior_counters.items():
                if count > 0:
                    print(f"{bt}:{count} ", end="")
            print()
